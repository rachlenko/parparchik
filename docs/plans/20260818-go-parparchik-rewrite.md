...
- [x] `internal/authz`: `Authorizer.Allowed(Principal, Action, repo) bool` — deny-by-default RBAC primitives (`Role`, `Grant`, `Principal`) that sit *on top of*, not as a replacement for, `PARPARCHIK_API_KEYS`/`AuthConfig` (user chose "standalone primitives only" over full httpapi wiring, given this is security-sensitive auth code with real architecture choices — OIDC library, session model — better made against a concrete requirement)
- [ ] OIDC/SSO integration for interactive/UI use cases — **not implemented**, deferred entirely per the user's explicit choice; needs a concrete identity provider this project doesn't have, and `Principal` is deliberately authentication-mechanism-agnostic so adding one later never touches this package
- [x] keep the existing `AuthConfig` API-key path as a "service account" tier alongside the new user/role model, don't remove it — `ServiceAccounts` (API key → `Principal`) plus `SuperuserRole()` (a `Role` reproducing today's all-or-nothing access exactly, for a deployment that hasn't adopted per-key roles yet) is the bridge; `AuthConfig` itself is completely unchanged
- [ ] wiring `Authorizer` into httpapi's live request path — **not implemented**, per the user's explicit scope choice for this pass
- [x] write tests (permission matrix cases including wildcard-repo/exact-repo/multi-role-union/unregistered-role-name/deny-by-default, `ServiceAccounts` lookup, and — added during review — nested-slice aliasing regressions for both constructors)
- [x] run project tests - must pass before next task
- ⚠️ Fixed during review: this is authorization code, so both **go-reviewer** and **security-reviewer** reviewed it in parallel (per this project's security.md: auth code requires security-reviewer). Both independently found the same root-cause HIGH issue from different angles: `NewAuthorizer` and `NewServiceAccounts` only shallow-copied their input — a `Role`'s `Grants`, a `Grant`'s `Actions`, and a `Principal`'s `Roles` are slices, so a caller mutating one of those slices *in place* after construction (through whatever reference they originally built it with) could silently change an already-built `Authorizer`'s decisions, turning a deny into an allow. security-reviewer additionally reproduced this as a genuine `go test -race` data race via the package-level `SuperuserRole` value (a shared mutable `var` whose nested slices any caller mutating "their own copy" was actually still aliasing). Fixed: both constructors now deep-copy every nested slice; `SuperuserRole` became `SuperuserRole()`, a function returning an independent `Role` per call (plus an exported `SuperuserRoleName` constant), so there is no shared mutable package-level authorization state at all. Also fixed (MEDIUM, security-reviewer): `NewAuthorizer` now logs a warning on a duplicate `Role.Name` instead of silently keeping the last one with no signal. Added regression tests for all of the above; coverage stayed at 100%.

### Task 33: Webhooks & event notifications
- [x] `internal/webhook`: `Registry` (mutex-guarded Subscriber bookkeeping, filtered by `EventType`) + `Dispatcher.Fire` — fires on `upload`/`relocate`/`policy_decision` (Task 28)/`scan_complete` (Task 25) `EventType`s to configurable subscriber URLs, concurrently across subscribers. Deliberately a dependency-free leaf package (`Event.Payload` is `any`, not `policy.Verdict`/`scan.Result` directly) — same "standalone, tested primitive, not wired to a live event source" scope as several packages built earlier this session, since there's no config surface yet for an operator to register a real subscriber
- [x] retry with backoff (exponential doubling, injectable `Sleep` seam for deterministic tests), per-subscriber **per-attempt** delivery status (`DeliveryStatus` — the full attempt history, not just the final outcome) — plus, beyond the plan's original bullet: optional HMAC-SHA256 payload signing (`X-Webhook-Signature`) when a `Subscriber.Secret` is configured, a documented (not implemented — deliberately, see below) SSRF consideration on subscriber URLs
- [x] write tests (fake HTTP subscriber via `httptest`, retry-on-failure with backoff-doubling assertion, exhausted-retries, multi-subscriber concurrent delivery, event-type filtering, HMAC signature presence/absence, context-cancellation-mid-retry, malformed/unmarshalable `Payload`, nested-slice aliasing on both the write path (`Register`) and read path (`Subscribers`))
- [x] run project tests - must pass before next task
- ⚠️ Fixed during review: no CRITICAL/HIGH findings (Approve) — go-reviewer specifically traced the concurrent `Fire`/`deliver` goroutine+channel pattern (one buffered channel slot per subscriber goroutine, exactly `len(subs)` receives) and confirmed it race-free under `-race -count=20`, confirmed the shared read-only marshaled `body []byte` has no mutation path across goroutines, and confirmed retry/backoff boundaries (exactly `MaxAttempts` attempts, no trailing sleep after the final attempt, cancellation checked only between attempts) match the documented design. One MEDIUM: `Registry.Subscribers` deep-copies the returned `Subscriber` struct but not its nested `Events` slice, so a caller mutating a returned `Subscriber.Events` in place could silently corrupt the Registry's own stored filter with no lock held and no call back into the package — the same aliasing bug class `internal/authz` was fixed for earlier this session, this time on `webhook`'s *read* path rather than a constructor. Fixed by deep-copying `Events` in `Subscribers` too; added a regression test. One LOW: added a doc-comment clarification that an already-canceled context still results in exactly one real delivery attempt (not zero) before `deliver` returns, since the cancellation check only runs between attempts.

### Task 34: Reporting, dashboards & cross-repo search
- [x] `internal/reporting` (new package, not an `httpapi` extension): `Record` joins `catalog.Entry` with optional `license.Result`/`scan.Result`/`*policy.Verdict`; `Search(records, Query)` filters by name substring (case-insensitive), exact version, license SPDX ID, vulnerability status, and minimum severity — deliberately a dependency-*heavy* package (unlike the leaf packages built earlier this session), since joining is its entire purpose
- [x] exportable reports (CSV/JSON) summarizing policy violations (`PolicyViolationsReport` — a nil `PolicyVerdict` is excluded as absent data, not treated as an implicit allow or counted as a violation), license breakdown (`LicenseBreakdown`, grouped by SPDX ID, deterministically sorted by count desc then SPDX ID asc), storage usage per repository (`StorageUsagePerRepository` — the one report computable from real, unenriched `catalog.Entry` data alone, no join needed)
- [x] no web UI is in scope here — this is API-only, matching this project's current backend-service shape; a UI would be a separate, explicitly-scoped effort. Even the *API* side isn't wired to an HTTP endpoint in this pass: same "standalone, tested primitive" scope as several packages built earlier this session, and for a sharper reason here — there is no enriched `[]Record` data source anywhere in this codebase yet (nothing attaches license/scan/policy results to a catalog entry), so an endpoint would have nothing real to search over regardless of whether the route itself existed
- [x] write tests (exact-vs-substring search semantics, empty-Query/empty-SPDXID edge cases, three-value policy Decision handling, sort determinism on ties, CSV column-to-header alignment for all three exports, nil-vs-recorded-Allow distinction surviving real CSV round-trip)
- [x] run project tests - must pass before next task
- ⚠️ Fixed during review: no CRITICAL/HIGH findings (Approve) — go-reviewer independently cross-checked `PolicyViolationsReport`'s three-value `Decision` handling against `policy.Decision`'s actual enum values, verified `Search`'s exact-vs-substring distinction, verified both directions of the empty-Query/empty-SPDXID edge cases, verified `LicenseBreakdown`'s tiebreak sort is a genuine total order (not relying on `sort.Slice`'s non-stability), and verified all three CSV writers' column-index-to-header alignment index-for-index (an off-by-one here would silently mislabel real report data). Two LOW, non-blocking style nits applied anyway since they were cheap: `StorageUsagePerRepository` dropped a redundant first-seen-order bookkeeping slice that the final sort-by-Bucket made pointless; `Search`'s result slice now pre-allocates to `len(records)` (left `PolicyViolationsReport`'s un-pre-allocated, since pre-allocating a filter's result to full input length is the worse trade-off when the result is typically much smaller than the input).

### Task 35: Backup & disaster recovery
- [x] document and script a backup procedure: [`docs/backup-and-disaster-recovery.md`](../backup-and-disaster-recovery.md) + `scripts/backup.sh`/`scripts/restore.sh` (also `make backup`/`make restore`) — mirror bucket contents to/from a local directory via the MinIO client, since each bucket's manifest already makes "back up the buckets" the complete story
- [x] verified this holds once Tasks 24/28/29/30/31/33 (all completed or design-docced this session) add state beyond "the manifest": audited every one explicitly in the doc above. One real, documented gap found — `internal/policy.AuditLog` (Task 28) has no persistence at all and is lost on restart, relevant once that package is wired into a live request path (not yet, so nothing is actually at risk in production today); proxy caches (Task 24) and replication (Task 30) are both covered by "back up the buckets" (replication is additionally independently re-derivable, being pull-only/idempotent); `internal/cleanup` (Task 29), `internal/webhook.Registry`/`internal/authz` (Tasks 32/33) introduce no durable state needing backup (deletion is inherently non-durable by design; the latter two are config-shaped, not artifact-derived); Task 31 is a design doc only this session, no new state
- [x] snapshot/restore tooling for the catalog cache: `internal/snapshot` — `Capture`/`Restore`, explicitly documented as a startup-time optimization only (not a durability mechanism), reusing `catalog.ManifestForBucket`/`LoadManifests` rather than a second, parallel load path — the exact same method `resolver.Bootstrap` itself calls
- [x] write tests (restore-from-manifest produces the same catalog state as a live sync — `TestRestore_ProducesTheSameCatalogStateAsALiveSync` populates a catalog via `Register` the same way `resolver.SyncRegistry` does, captures, JSON-round-trips, restores into a fresh catalog, and asserts `ListAll()` is identical; plus version-rejection, malformed-JSON, replaces-existing-contents, and empty-catalog cases)
- [x] run project tests - must pass before next task
- ⚠️ Fixed during review: no CRITICAL/HIGH findings (Approve) — go-reviewer independently read `catalog.Catalog.LoadManifests`/`Register` and `resolver.Bootstrap` (rather than trusting the doc comment's claim) to confirm `Restore` genuinely reuses the identical code path `Bootstrap` does, and empirically verified (via a throwaway test, deleted afterward) that a same-key-different-bucket scenario round-trips correctly — concluding this can't actually produce a cross-bucket conflict during `Restore` at all, since `Capture` only ever captures the single already-priority-resolved winning entry per key (a natural consequence of `Catalog`'s map-backed storage), so the "identical conflict resolution" doc claim is true but slightly overstates what's being exercised. Only LOW/informational findings, all left as-is (a `Restore`-target-catalog-with-different-bucketType-config edge case; an already-accepted-category unreachable error branch; two same-valued-but-independent `Version` fields worth a reader's awareness, not a bug).

**This was the last task in the plan.** All 35 tasks are now `[x]` or explicitly, deliberately scoped-out (Task 23 skipped per user choice; Task 31's implementation deferred pending its own design doc's phasing recommendation; several tasks' "wire into a live request path" bullets remain open by explicit user choice — see each task's own notes above for what's deferred and why). Nothing in this session's work has been pushed to origin.
